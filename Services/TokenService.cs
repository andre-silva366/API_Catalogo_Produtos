...
public class TokenService : ITokenService
{
    ...
    {
        // Pega a palavra secreta do arquivo de configuração
        var key = _config.GetSection("JWT").GetValue<string>("SecretKey") ?? throw new InvalidOperationException("Invalid secret key");

        // Converte a palavra em um array de bytes
        var privateKey = Encoding.UTF8.GetBytes(key);

        // Gera as credenciais usando a palavra chave e o algoritmo de incriptação
